...
void gradientDescent(float *sLo, float *sHi, int sensorSamples, int dir, int v0, float G, float ls, float lw, int ks, float sc, float P, float *rotMemory) {
  ...
  float randCont = sin(randRotation*PI)*(1-P)/ls; // contribution from random walk
  float gradCont = P*tanh(50*(float)dir*ks*(signalLeft-signalRight))/ls; // contribution from gradient
   if(abs(signalLeft-signalRight)<0.01){
    gradCont = 0;
  }
  float omega = gradCont + randCont;
...
// probe object by moving towards it and measuring sensor values
int probeObject(float *sensorValues, float N, int rantDetectionThreshold, float vProbe, unsigned long tProbe){
  unsigned long timer  = millis();
  int state = 1;
  setMotors(vProbe,vProbe);
  while(millis()<timer+tProbe){
    checkBumpSensor(sensorValues,N);
    if(sensorValues[0]<rantDetectionThreshold){
      state = -1;
      break;
    }
  }
