...
#include <Servo.h>
#include <math.h>
#include "customized_routines/routines_rant7.h" // select correct rant number
...
template <typename T> int sgn(T val) { // sign function template
  ...
  // system parameters
    ...
    float P = 1; // Cooperation parameter. P=0 fully random taxis, P=1 only chemotaxis.
    ...
    int rantDetectionThreshold = 900;
    float vProbe = 50; // speed of obstacle probing
    unsigned long tProbe = 1000; // duration of probing
...
void loop() {
  ...
  if(dir==1 && svMean<pheromoneThresholdBndry){
    ...
    if(ks*svMean*P>ks*(phMean+ks*phDiff)){
      int state = probeObject(bumpSensor, bumpSensorSamples, rantDetectionThreshold, vProbe, tProbe);
      if(state==1){
        setMotors(0, 0);
        delay(2000); // pause before pick-up routine
        ...
        delay(2000);    
        int randTurnSign = sgn(random(-1000,1000));
        setMotors(-60*randTurnSign, 60*randTurnSign);
        delay(1500);
      }
